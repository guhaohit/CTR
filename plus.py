...
def data(f_train, D, dayfilter = None, dayfeature = True, counters = False):
    ...
    for t, row in enumerate(DictReader(f_train)):
        ...
        if row[key1] in inter_dic:
            if row[key2] in inter_dic[row[key1]]:
                inter_value = inter_dic[row[key1]][row[key2]]
        #print inter_value

        inter = abs(hash(str(row[key1]) + '_' + str(row[key2]) + '_' + str(inter_value))) % D
...
""")
    ...
    parser.add_argument('-t', "--train", default = "train.csv")
    parser.add_argument('--test', default = "test.csv")
    parser.add_argument('-p', "--predictions", default = "submission1234.csv")
...
if __name__ == "__main__":
    inter_dic = {}
    output = open('LR-onehot5.pkl', 'rb')
    inter_dic = pickle.load(output)
